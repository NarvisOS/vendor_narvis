...
func init() {
	android.RegisterModuleType("narvis_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = narvisExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = narvisExpandVariables(ctx, dep_file)
	...
	cmd := narvisExpandVariables(ctx, String(g.properties.Cmd))
