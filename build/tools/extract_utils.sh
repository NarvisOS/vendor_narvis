...
function setup_vendor() {
    ...
    export NARVIS_ROOT="$3"
    if [ ! -d "$NARVIS_ROOT" ]; then
        echo "\$NARVIS_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$NARVIS_ROOT/$OUTDIR" ]; then
        mkdir -p "$NARVIS_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$NARVIS_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$NARVIS_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$NARVIS_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$NARVIS_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR The NARVIS Project\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR The NARVIS Project\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR The NARVIS Project\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The NARVIS OS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The NARVIS OS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The NARVIS OS Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The NARVIS OS Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The NARVIS Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$NARVIS_ROOT"/prebuilts/tools-narvis/common/smali/baksmali.jar
        export SMALIJAR="$NARVIS_ROOT"/prebuilts/tools-narvis/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$NARVIS_ROOT"/prebuilts/tools-narvis/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$NARVIS_ROOT"/prebuilts/tools-narvis/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$NARVIS_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$NARVIS_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$NARVIS_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$NARVIS_ROOT"/vendor/narvis/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$NARVIS_ROOT"/"$OUTDIR"/radio
