...
if __name__ == '__main__':
    # Default to Narvis Gerrit
    default_gerrit = 'https://review.Narvisgzr.com'
    ...
    for item in mergables:
        ...
        if args.gerrit == default_gerrit:
            ...
            if args.pull:
                cmd = ['git pull --no-edit Narvis', item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch Narvis', item['fetch'][method]['ref']]
            if args.quiet:
                cmd.append('--quiet')
        ...
        if args.gerrit != default_gerrit or os.stat(FETCH_HEAD).st_size == 0:
            # If not using the default gerrit or Narvis failed, fetch from gerrit.
            if args.verbose:
                if args.gerrit == default_gerrit:
